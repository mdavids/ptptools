 ...
 *
 * Adapted for own use by SIDN Labs, Marco Davids
 * Far from perfect yet. Does, for instance, not take into account that
 * we run UDPv6 and UDPv4 on one interface. Or two instances of ptp4l 
 * on one machine. So don't be fooled by the results!
 */
...
/* This is very specific to our choices (will become something like "/var/run/ptp4leno1v6") */
char ptp_sock[25];
int  ptp_sock_size = 21;
...
static int check_ptp_offset(int verbose)
{
